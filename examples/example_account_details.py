...
from yapily import StatementsApi
...
def main():
    ...
    if (consent.data.status == 'AUTHORIZED'):
        ...
        try:
            transactions = transactionsApi.get_transactions_using_get(accounts.data[0].id ,consent_token ) #accounts.data[0]._id)
            print("**************TRANSACTIONS**************")
            print(transactions)
            print("****************************************")

        except:
            print('Could not open account ' + accounts.data[0].id)

        statementApi = StatementsApi(apiClient)
        
        statements = statementApi.get_statements_using_get(consent_token , accounts.data[0].id) ##accounts.data[0]._id)
        statement = statementApi.get_statement_using_get(consent_token , accounts.data[0].id,statements.data[0].id) ##accounts.data[0]._id)
        stafile = statementApi.get_statement_file_using_get(consent_token , accounts.data[0].id , statements.data[0].id) ##accounts.data[0]._id)
        print("**************STATEMENT**************")
        print(stafile)
        print("****************************************")
